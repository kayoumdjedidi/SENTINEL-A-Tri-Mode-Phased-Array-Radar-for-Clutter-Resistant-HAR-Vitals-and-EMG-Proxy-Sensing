#!/usr/bin/env python3
"""Baseline FMCW FFT + waterfall for CN0566 over WSL -> Pi -> Pluto.

This stays intentionally close to ADI's `RADAR_FFT_Waterfall.py` so you can
validate the hardware chain with minimal unknowns before using the richer
SENTINEL runtime.

This script preserves ADI's known-good PLL setting but names the frequencies
explicitly:
...
# ---------------------------------------------------------------------------
# Baseline parameters: keep these close to ADI's reference
# ---------------------------------------------------------------------------
...
PLL_OUTPUT_FREQ = 12.145e9
RF_OUTPUT_FREQ = PLL_OUTPUT_FREQ - CENTER_FREQ - SIGNAL_FREQ
CHIRP_BW = 500e6
NUM_STEPS = 1000
RAMP_TIME_US = 1.2e3
...
print(
    "[baseline] rf_center=%.4fGHz pll_center=%.4fGHz sample_rate=%.1fMHz "
    "ramp=%dus bw=%.0fMHz"
    % (
        RF_OUTPUT_FREQ / 1e9,
        PLL_OUTPUT_FREQ / 1e9,
...
class Window(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Baseline RADAR FFT Waterfall")
    ...
    def _build_ui(self):
        ...
        title = QLabel("PHASER Baseline FMCW Radar")
        font = title.font()
        ...
        self.waterfall.setRange(yRange=(SIGNAL_FREQ, SIGNAL_FREQ + 20e3))
    ...
    def update_display(self, data):
        ...
        spectrum = np.abs(np.fft.fftshift(np.fft.fft(combined * window)))
...
print("Baseline FMCW waterfall running. Close the window or press Quit to stop.")
ret = app.exec()
